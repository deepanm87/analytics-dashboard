...
export async function AnalyticsInfo() {
    ...
    const infoItems: InfoItem[] = [
        ...
        { id: "updatedAt", label: "Updated At", value: updatedAt ? new Date(updatedAt).toLocaleDateString() : "" }
    ]
        ...
        <div className="flex-gap-2">
            { infoItems.map( item => (
                <Badge
                    key={item.id}
                    variant="outline"
                    className="text-sm font-bold text-black bg-pink-300 inline-block px-2 py-1 border-2
                        border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mr-2"
                >
